...
namespace cartesian_adaptive_compliance_controller
{
  ...
  rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn CartesianAdaptiveComplianceController::on_init()
  {
  ...
  controller_interface::return_type CartesianAdaptiveComplianceController::init(const std::string &controller_name)
  {
  ...
  rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn CartesianAdaptiveComplianceController::on_configure(
  ...
  rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn CartesianAdaptiveComplianceController::on_activate(
      const rclcpp_lifecycle::State &previous_state)
  {
  ...
  rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn CartesianAdaptiveComplianceController::on_deactivate(
      const rclcpp_lifecycle::State &previous_state)
  {
    using TYPE = rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn;
  ...
  controller_interface::return_type CartesianAdaptiveComplianceController::update(const rclcpp::Time &time,
  ...
  void CartesianAdaptiveComplianceController::ftSensorWrenchCallback(const geometry_msgs::msg::WrenchStamped::SharedPtr wrench)
  {
    KDL::Wrench tmp;
  ...
  ctrl::Vector6D CartesianAdaptiveComplianceController::computeStiffness()
  {
    ...
    real_t A[5 * 3] = {
      ...
      0, x_d(1) - x(1), 0,
      0, 0, x_d(2) - x(2),
      position_error(0) * velocity_error(0), position_error(1) * velocity_error(1), position_error(2) * velocity_error(2),
      position_error(0) * velocity_error(0), position_error(1) * velocity_error(1), position_error(2) * velocity_error(2)
    };    
    
    real_t ubA[5] = 
    { 
      F_max(0) - m_damping(0,0) * velocity_error(0), 
      F_max(1) - m_damping(1,1) * velocity_error(1), 
      F_max(2) - m_damping(2,2) * velocity_error(2), 
      1e9, 
      1e9
    };
    ...
    real_t lbA[5] = 
    {
      F_min(0) - m_damping(0,0) * velocity_error(0), 
      F_min(1) - m_damping(1,1) * velocity_error(1), 
      F_min(2) - m_damping(2,2) * velocity_error(2), 
      T_constr_min, 
      T_dot_min
    };
...
PLUGINLIB_EXPORT_CLASS(cartesian_adaptive_compliance_controller::CartesianAdaptiveComplianceController, controller_interface::ControllerInterface)